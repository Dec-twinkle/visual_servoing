...
class Task():
    ...
    def addHandeyeNoise(self,euler_sigma,t_sigma):
        ...
        euler[0] = euler[0]+random.gauss(0,euler_sigma)
        euler[1] = euler[1]+random.gauss(0,euler_sigma)
        euler[2] = euler[2]+random.gauss(0,euler_sigma)
        self.simu_eMc[:3,:3] = t3d.euler.euler2mat(euler[0],euler[1],euler[2])
    ...
    def transformCameraToRobotVelocity(self,cameraVelocity):
        ...
        matrix = np.dot(self.eMc,np.dot(matrix,np.linalg.inv(self.eMc)))
